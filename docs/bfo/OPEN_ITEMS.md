...
| id | item | closed |
|---|---|---|
| OI-8 | Two folk classes sat under a Schwartz value the corpus itself does not place them under | 2026-09-17 by D-015. `CreativityDisposition` moved to Schwartz Self-Direction, whose defining goal is independent thought and action and whose items include creativity, with a related match to Stimulation for the novelty it shares. `RespectDisposition` moved to the general parent, because no one Schwartz value covers regard for others' feelings, wishes, rights and traditions, with related matches to Universalism, Conformity and Tradition. Neither definition was bent to fit a parent |
