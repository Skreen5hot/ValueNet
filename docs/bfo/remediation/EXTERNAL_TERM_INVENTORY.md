...
| `https://www.commoncoreontologies.org/ont00000958` | CCO Information Content Entity | Ancestor of the selector, evidence-record and moral-epistemics information classes; not of textual representations or spans since D-005 | None before Phase 3 | **Verified and supplied** — CCO 2.2 defines it as a GDC about some entity; included with its equivalence and aboutness dependency | Use as the verified information-content genus; do not type carriers as ICEs |
| `https://www.commoncoreontologies.org/ont00001921` | CCO has input | Restriction property for informational inputs, used directly since D-007 | Added in Phase 1 | **Verified, supplied, and reused** — CCO 2.2 ranges over the union of specifically dependent continuant, generically dependent continuant, and independent continuant that is not a spatial region; CCO 2.0 over `continuant`. Neither admits a process, so an ICE input is in range under both (corrected 2026-09-16; formal review N1) | Used directly with the ICE filler stated in each restriction and as the SHACL path; `hasInformationalInput` retired (D-007, implemented 2026-09-16) |
| `https://www.commoncoreontologies.org/ont00001986` | CCO has output | Restriction property for informational outputs, used directly since D-007 | Added in Phase 1 | **Verified, supplied, and reused** — CCO 2.2 ranges over the union of specifically dependent continuant, generically dependent continuant, and independent continuant that is not a spatial region; CCO 2.0 over `continuant`. Neither admits a process, so an ICE output is in range under both (corrected 2026-09-16; formal review N1) | Used directly with the ICE filler stated in each restriction and as the SHACL path; `hasInformationalOutput` retired (D-007, implemented 2026-09-16) |
| `https://www.commoncoreontologies.org/ont00000686` | CCO Designative Information Content Entity | Superclass of `TextSpanSelector` | Added in the D-005 extract revision, 2026-09-16 | **Verified and supplied** — CCO 2.2 defines it as an ICE that consists of a set of symbols that designate some entity, equivalent to ICE and `designates` some entity, and disjoint with Descriptive ICE | Retain the superclass axiom (D-005 item 8) |
| `https://www.commoncoreontologies.org/ont00001916` | CCO designates | Superproperty of `selectsTextSpan` | Added in the D-005 extract revision, 2026-09-16 | **Verified and supplied** — domain Designative ICE, range entity, a subproperty of `is about` (`ont00001808`); inverse `designated by` (`ont00001879`) is supplied with it | Retain `selectsTextSpan rdfs:subPropertyOf` this property; never assert it, or any other aboutness, with a textual representation or span as subject (D-005) |
| `https://www.commoncoreontologies.org/ont00001765` | CCO has text value | Deliberately not used | Never supplied | **Verified against the v2.2 release, not supplied** — domain Information Bearing Entity; on a form-level representation it is inconsistent (`tests/bfo/test_text_value_placement.py`) | Use `vn-core:hasTextualSequenceValue` on representations and spans (D-005 items 5 and 6) |
...
| `https://www.commoncoreontologies.org/ont00000005` | Act | Superclass of `MoralAssessmentAct` since D-008, inherited by discernment and rash judgment | **Verified, supplied, and reused** — a process in which at least one agent plays a causative role; not planned | Retain; the generic parent that admits spontaneous judgment |
| `https://www.commoncoreontologies.org/ont00000636` | Act of Appraisal | Superclass of `MoralDiscernmentAct` since D-008; Phase 6 superclass of `MoralAssessmentAct` until then | **Verified, supplied, and reused** — Act of Measuring that evaluates or judges nature, value, importance, condition, or quality | Retain the superclass path; equivalence rejected because the CCO extension is broader |
| `https://www.commoncoreontologies.org/ont00000853` | Descriptive Information Content Entity | Superclass of `ObservationalEvidenceICE`, `MoralAssessmentICE` and, since R10 of the 2026-09-16 review response, `ValueEvidenceAnnotation`; inherited by safety assessments and culpability ascriptions | **Verified, supplied, and reused** — defined as an ICE that describes some entity; disjoint with Prescriptive ICE | Retain the superclass paths and imported disjointness |
