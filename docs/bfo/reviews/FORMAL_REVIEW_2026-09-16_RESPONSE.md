...
| R18 | Non-folk done; folk after D-014 is implemented. Every class and property in core, Schwartz, Moral Foundations and Moral Epistemics now carries a comment and an example: 32 comments and 60 examples written, worked examples taken from the module's own scenario where one exists. The folk gap, 134 of each, is recorded and gated; D-014's removals take it to 128 | D-013; `tests/bfo/test_definition_discipline.py` |
