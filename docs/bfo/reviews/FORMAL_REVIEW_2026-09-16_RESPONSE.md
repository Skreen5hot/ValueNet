...
| R15 | **Done.** The owner adopted membership criteria M1–M7 and coverage reported by kind: of the 278 corpus values, 98 have an exact class, 9 become alternative labels, 146 are correspondences in the mapping layer, 25 are excluded and none is pending. Seven folk IRIs are retired — the three Schwartz copies, `OpennessDisposition` (superseding D-012 item 3), `ImpactDisposition`, `DiscretionDisposition` and `ResourcefulnessDisposition` — five classes are re-parented and six added. The record, with its research and a row for each of the 186 unpaired corpus values and 45 unpaired classes, is revision 4 of `docs/bfo/remediation/R15_FOLK_MEMBERSHIP_PROPOSAL.md`, and the ontology is checked against its tables. The coverage tool measures the four kinds from the ontology. The folk curation it unblocked, 75 genus corrections and 128 comments and examples, is done under D-013 | D-014; `tests/bfo/test_folk_membership.py`, `tests/bfo/test_folk_coverage.py` |
| R16 | Done; gated. Every definition in all five modules opens with an asserted parent. Folk's 84 were recorded by name with equality as the gate; D-014 removed five and re-parented four, and the curation pass corrected the other 75, so the record is empty and any new misalignment fails. Moral Epistemics measured 3, not 6: phase D had fixed one, and two were the measure reading "ICE" in a label as different from "information content entity" in the definition | `tests/bfo/test_definition_discipline.py`; phase E |
...
| R18 | Done. Every class and property in all five modules carries a comment and an example. Non-folk: 32 comments and 60 examples, worked examples taken from the module's own scenario where one exists. Folk: after D-014's removals took the gap from 134 to 128, the 128 were written, each comment saying what separates the class from its nearest neighbours and which corpus words correspond to it. The gate's record is empty. Eight folk definitions changed beyond their genus; D-013 lists them | D-013; `tests/bfo/test_definition_discipline.py` |
