...
BFO layer is deliberately excluded: it has been gone over repeatedly and would
mostly produce findings already known, at full price.

Five agents, a real Adjudicator, all six phases, real API calls. Every finding
...
def main(argv=None) -> int:
    ...
    built = ingest.build("valuenet-run1", "2026-08-20", "2026-08-26", repo=repo,
                         ontology=True, ontology_scopes=SCOPES, include_github=False)
    if built.errors:
        print(f"substrate invalid: {built.errors[0]}", file=sys.stderr)
        return 2
    sub_path = ingest.write(built, out / "RUN1_INPUT.yaml")
    substrate = Substrate.load(sub_path)

    print(f"\nRUN 1  the unaudited layer: {', '.join(SCOPES[:3])}, …")
    print(f"  substrate  {len(substrate)} records "
          f"({built.counts.get('document',0)} document, {built.counts.get('metric',0)} metric)")
    print(f"  gaps       {substrate.unavailable_types()}")
    key_metrics = [r for r in built.document["records"]
                   if r["type"] == "metric" and r["payload"]["check"].startswith("files_")]
    for m in key_metrics[:8]:
        print(f"    {m['ref']:52} {int(m['payload']['value'])}")
    ...
    # Two backends, not one. They answer different questions and, until this
    # run, sharing an instance silently handed the Adjudicator an agent's
    # prompt helpers.
    ...
    rt = Runtime.initialize("valuenet-run1", substrate, roster=roster,
                            state_path=out / "RUN1_STATE.yaml")
    adj = Adjudicator(rt, adj_backend)

    # ---- Phase 1 -------------------------------------------------------
    print("\nPHASE 1  gathering")
    for agent in build_roster(rt, backend):
        ...
        show(f"{agent.name} submits", agent.gather(tokens=4000, findings=findings))
    ...
    for agent in build_roster(rt, backend):
        ...
        show(f"{agent.name} submits", agent.evaluate(tokens=3500, assessments=assessments))
    ...
    print("\nPHASE 5  actions")
    for agent in build_roster(rt, backend):
        res = agent.propose_actions(tokens=2500)
        if res:
    ...
    for a in list(rt.state.get("actions") or []):
        show(f"accept {a['id']}", rt.submit({
            ...
            "actions": [{"id": a["id"], "status": "accepted"}]}, "Corpus"))
    ready, unmet = rt.exit_ready()
    if unmet:
        print(f"  --  {unmet[0][:110]}")
