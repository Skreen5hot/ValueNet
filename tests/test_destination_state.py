...
def test_every_declared_regeneration_is_handled():
    ...
    An allowance with `discharge: regenerate` is work a program does. If
    a new one appears and REGENERATORS is not updated, the checker would
    materialise a state the plan never intends and report it green --
    which is how folk_aligned.ttl came to carry a header naming a
    generator that had moved. Unhandled is allowed; unnoticed is not, so
    an entry mapped to None still has to be written down.

    Only the regenerate kind. `discharge: edit` is a line somebody
    rewrites in the move commit, and demanding an automated remedy for
    those left the choice between failing forever and printing a note
    ...
    assert not missing, (
        'these allowances are discharged by regenerating but no '
        'REGENERATORS entry says with what, or that it is manual: '
        + str(missing))
...
def test_every_expiring_allowance_says_how_it_is_discharged():
    ...
    Without the distinction the two were one obligation, and the checker
    demanded a generator for 28 literals that no generator produces."""
    from marep import layout
...
def test_edits_and_regenerations_do_not_overlap():
    """One obligation, one remedy. Both would leave it ambiguous
    which of the two proves the wave was completed."""
