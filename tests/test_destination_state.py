...
from marep.layout import component, relative, repository_root  # noqa: E402
...
def test_every_consumer_snippet_has_its_placeholders_substituted():
    ...
    # Through the contract. A literal here is one more occurrence that
    # goes stale in the original-valuenet wave, and an allowance for a
    # path this test never actually resolves would be an exception
    # granted for nothing.
    gen = relative(component('tool.generate-folk-aligned').resolve())
...
#: A standing allowance list, so these assertions survive the migration.
#:
#: The real list shrinks as waves complete -- that is what `remove_after_wave`
#: means -- so a test asserting "something is owed after the bfo wave" would
#: start failing the moment the bfo wave removed the last of them, reporting a
#: broken checker when the checker had done its job.
SYNTHETIC = [
    {"id": "syn-regen", "file": "gen/out.ttl", "literal": "old/tool.py",
     "category": "generated-metadata", "owner": "unowned",
     "justification": "synthetic", "permanent": False,
     "discharge": "regenerate", "remove_after_wave": "bfo"},
    {"id": "syn-edit", "file": "src/thing.py", "literal": "old/doc.md",
     "category": "cross-reference", "owner": "unowned",
     "justification": "synthetic", "permanent": False,
     "discharge": "edit", "remove_after_wave": "original-valuenet"},
    {"id": "syn-permanent", "file": "src/thing.py", "literal": "kept/x.ttl",
     "category": "historical-record", "owner": "unowned",
     "justification": "synthetic", "permanent": True},
]


@pytest.fixture
def synthetic_allowances(monkeypatch):
    from marep import layout
    monkeypatch.setattr(layout, "path_allowances", lambda: list(SYNTHETIC))
    return SYNTHETIC
...
def test_every_declared_regeneration_is_handled():
    ...
    An allowance with `discharge: regenerate` is work a program does. If a
    new one appears and REGENERATORS is not updated, the checker would
    materialise a state the plan never intends and report it green -- which
    is how folk_aligned.ttl came to carry a header naming a generator that
    had moved. Unhandled is allowed; unnoticed is not, so an entry mapped to
    None still has to be written down.

    Against the live list, because the point is to catch an entry somebody
    adds tomorrow. It goes quiet when the last one is retired, which is
    correct: there is then nothing to handle.
    """
    from marep import layout
    ...
    assert not missing, (
        "these allowances are discharged by regenerating but no REGENERATORS "
        "entry says with what, or that it is manual: " + str(missing))


def test_every_expiring_allowance_says_how_it_is_discharged():
    """Regenerated by a program, or edited in the move commit."""
    from marep import layout
    undeclared = [a["id"] for a in layout.path_allowances()
                  if a.get("remove_after_wave")
                  and a.get("discharge") not in ("regenerate", "edit")]
    assert not undeclared, undeclared


def test_owed_regenerations_track_the_wave(synthetic_allowances):
    """Nothing is owed before its wave; everything is owed at the end."""
    early = {a["id"] for a in CHECKER.owed_regenerations("bfo")}
    assert early == {"syn-regen"}, early
    late = {a["id"] for a in CHECKER.owed_regenerations(None)}
    assert late == {"syn-regen"}, late
    assert "syn-permanent" not in late, "a permanent entry can never be owed"


def test_pending_edits_track_the_wave(synthetic_allowances):
    assert CHECKER.pending_edits("bfo") == []
    late = {a["id"] for a in CHECKER.pending_edits(None)}
    assert late == {"syn-edit"}, late


def test_edits_and_regenerations_do_not_overlap(synthetic_allowances):
    """One obligation, one remedy. Both would leave it ambiguous which of
    the two proves the wave was completed."""
    reg = {a["id"] for a in CHECKER.owed_regenerations(None)}
    edits = {a["id"] for a in CHECKER.pending_edits(None)}
    assert not (reg & edits), sorted(reg & edits)
    assert reg and edits, (reg, edits)


def test_an_undischargeable_obligation_fails_rather_than_notes(
        tmp_path: Path, monkeypatch, synthetic_allowances):
    """The blocking defect from the step 4 gate: printed as a note beside a
    green verdict.
