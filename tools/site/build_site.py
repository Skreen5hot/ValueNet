...
def main(argv=None) -> int:
    ...
    # Generated data first, because the module and download pages are
    # rendered from it. Produced into the artifact rather than committed,
    # and part of this build so "one deterministic build" stays true: a
    # site whose index is generated by a separate step can be deployed
    # with an index from a different commit.
    ...
    index_mod = load("build_class_index", "tools/site/build_class_index.py")
    rc = index_mod.main(["--index", str(out / "data/class-index.json"),
                         "--coverage", str(out / "data/coverage.json")])
    if rc:
        raise SystemExit("the class index build failed")

    # Downloads, for the same reason: a bundle produced by a separate step
    # can be deployed alongside pages from a different commit, and its
    # checksums would then describe neither.
    downloads_mod = load("build_downloads", "tools/site/build_downloads.py")
    downloads_mod.build(out, commit, int(epoch))

    fragments_mod = load("build_fragments", "tools/site/build_fragments.py")
    fragments = fragments_mod.build(out)
    ...
    for path in files:
        ...
        if path.suffix.lower() == ".html":
            ...
            text = fragments_mod.substitute(text, fragments)
    ...
    # The schemas, published at the path their $id claims. A schema that
    # names a URL the build does not serve is a dangling reference in the
    # one document whose job is to say what the data must look like.
    from marep import layout

    schema_src = layout.component("site.schemas").resolve()
    for schema in sorted(schema_src.glob("*.json")):
        dest = out / "schemas" / schema.name
        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_bytes(schema.read_bytes())

    # Generated trees. `downloads/` also receives index.html from the
    # source loop above, so already-recorded paths are skipped: counting a
    # file twice would put it in the tree digest twice and make the digest
    # depend on which loop happened to reach it.
    already = {rel for rel, _ in written}
    for extra in sorted(list((out / "data").rglob("*"))
                        + list((out / "downloads").rglob("*"))
                        + list((out / "schemas").rglob("*"))):
        if extra.is_file() and extra.relative_to(out).as_posix() not in already:
            os.utime(extra, (int(epoch), int(epoch)))
            written.append((extra.relative_to(out).as_posix(),
